<template>
  <div class="card-carousel">
    <client-only>
      <swiper ref="swiper" :options="swiperOptions" @transitionEnd="onSlideChange">
        <swiper-slide :class="[images.length==1?'mb-0':'mb-5']"  v-for="(image, index) in filteredImages" :virtualIndex="index" :key="index">
          <div :class="[portrait?'portrait-card-img':'card-img']"  style="overflow:hidden;">
          <v-img
            transition="xxx"
            :key="'main-img-'+index"
            :lazy-src="lazyGif"
            :ref="'main-img-'+index"
            :eager="(index===0 || postId)?false:eager"
            contain
    ...
    <GridCarousel v-model="activeImage" :filteredImages="filteredImages" :eager="eager" :postId="postId" :thumbnailWidth="thumbnailWidth"/>
...
export default {
  name: 'Carousel',
  props: ['isVisibleObject','startingImage', 'resizedImages','images', 'showProgressBar', 'thumbnailWidth', 'eager','portrait','postId'],
  components: {
    GridCarousel
  },
  data() {
