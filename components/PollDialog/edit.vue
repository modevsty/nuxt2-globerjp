...
export default {
  ...
  watch: {
    ...
    originalPoll: {
      handler(val) {
        this.poll = val
        this.expireDate = moment(parseInt(val.expireAt)).format('YYYY-MM-D')
        this.expireTime = moment(parseInt(val.expireAt)).format('HH:mm')
      },
      immediate: true,
      deep: true
    },
  ...
  methods: {
    ...
    async deletePoll() {
      await this.$fire.firestore.collection('polls').doc(this.poll.id).delete()
      .catch((error) => {
    ...
    async editPoll() {
      ...
      let data = {
        ...
        type: this.poll.type,
        creatorId: this.poll.creatorId,
        expireAt: Date.parse(this.expireDate+'T'+this.expireTime).toString(),
        creatorId: this.user.uid,
        createdAt: this.poll.createdAt || currDate,
        updatedAt: currDate,
      }
      // let data = {
      ...
      await this.$fire.firestore.collection('polls').doc(this.poll.id).update(data)
      .catch((error) => {
