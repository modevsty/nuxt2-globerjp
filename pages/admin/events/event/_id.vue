  ...
  margin: 0 auto; " type="error" :value="!!error">{{ error }}</v-alert>
              ...
              <v-container>
                ...
                <v-row>
                  ...
                  <v-checkbox
                    ...
                    :label="`Publish this event: ${event.isPublished.toString()}`"
                  ></v-checkbox>
                  </v-col>
          ...
          <v-card class="mt-5 mb-5">
            <client-only>
              ...
              :eventId="event.id"
              :roomAvatarUrl="event.images?.length?event.images[0].teaserImageUrl:''"
              />
            </client-only>
          </v-card>
          <!-- <v-card class="mt-5">
          ...
          <v-card v-if="attendees.length && userIdtoEntryDict">
            <input style="display:none;" id="myInput" v-model="emailList" />
            <div class="d-flex d-sm-block flex-wrap justify-space-between justify-sm-end">
              <v-btn style="float:right; margin:5px;" @click="copyEmail">Copy Emails</v-btn>
              <!-- <CsvDownloadBtn style="float:right; margin: 5px;" :original-csv-data="csvData" :original-labels="labels" :original-title="title"/> -->
              <vue-json-to-csv :json-data="csvData" :csv-title="title" :labels="labels" @error="csvDownloadErrorHandler">
                <v-btn style="float:right; margin: 5px;" @click.stop="e => {download()}">
                  Download CSV
                </v-btn>
                <v-btn ref="download-csv" style="display:none;" />
              </vue-json-to-csv>
            </div>
          <UserList :attendees="attendees" :userIdtoEntryDict="userIdtoEntryDict" :schema="event.formSchema"  class="mt-5 mb-5"/>
          </v-card>
        </div>
      </div>
    </v-container>
...
export default {
  ...
  methods: {
      updateData(val) {
        this.attendees = val.attendees
        this.entries = val.entries
        this.userIdtoEntryDict = val.userIdtoEntryDict
      },
      csvDownloadErrorHandler(e) {
        this.error = "Error: " + e
        setTimeout(()=>{
          this.error = null
        },5000)
      ...
      async download() {
        ...
        this.attendees.forEach(e => {
          const customFormValues = {...this.userIdtoEntryDict[e.id].customFormValues}
          const customFormElementNameToObjDict = {...this.userIdtoEntryDict[e.id].customFormElementNameToObjDict}
          let values = {}
          if (!(customFormValues
          ...
          && Object.getPrototypeOf(customFormValues) === Object.prototype)) {
            for (const [key, value] of Object.entries(customFormValues)) {
              if (! labels.hasOwnProperty(customFormElementNameToObjDict[key].name)) {
                labels[customFormElementNameToObjDict[key].name] = {
                  title: customFormElementNameToObjDict[key].label
                }
              }
              if (customFormElementNameToObjDict[key].type == 'file') {
                values = {
                  ...values,
                  [customFormElementNameToObjDict[key].name] : customFormValues[key].url
                }
              } else {
                let val = customFormValues[key]
                if (typeof customFormValues[key] === 'string') {
                  val = customFormValues[key].replaceAll('"','""')
                } else if (Array.isArray(customFormValues[key])) {
                  val = customFormValues[key].map(e => {
                    if (typeof e === 'string') {
                      return e.replaceAll('"','""')
                    }
                    return e
                ...
                values = {
                  ...values,
                  [customFormElementNameToObjDict[key].name] : val
                }
              }
        ...
        this.csvData = [...attendees]
        this.labels = {...this.labels,...labels}
        this.$forceUpdate()
        this.$nextTick(()=>{this.$refs['download-csv'].$el.click()})
      ...
      async getData() {
        this.entries = await this.$fire.firestore.collection('entries').where("eventId","==",this.$route.params.id)
        .get()
        .then((querySnapshot) => {
          let out = []
          querySnapshot.forEach((doc) => {
        ...
        this.userIdtoEntryDict = userIdtoEntryDict
